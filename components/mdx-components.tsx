...
export function useMDXComponents(components: MDXComponents): MDXComponents {
  return {
    ...
    a: ({ href, children }) => {
      ...
      return (
        <Link href={href || "#"} className="font-medium text-brand-rose-1 underline underline-offset-4">
          {children}
        </Link>
      )
    },
    ...
    blockquote: ({ children }) => (
      <blockquote className="mt-6 border-l-2 border-gray-7 pl-6 italic">{children}</blockquote>
    ),
    ...
    table: ({ children }) => (
      <div className="my-6 w-full overflow-y-auto">
        <table className="w-full overflow-hidden rounded-md border border-gray-11">{children}</table>
      </div>
    ),
    tr: ({ children }) => <tr className="m-0 border-t border-gray-11 p-0">{children}</tr>,
    th: ({ children }) => <th className="border border-gray-11 px-4 py-2 text-left font-bold">{children}</th>,
    td: ({ children }) => <td className="border border-gray-11 px-4 py-2 text-left">{children}</td>,
    code: ({ className, children, ...props }) => {
      ...
      return (
        <code className="relative rounded bg-gray-11 px-[0.3rem] py-[0.2rem] font-mono text-sm" {...props}>
          {children}
        </code>
      )
    },
